...
public class RightFootBehavior : MonoBehaviour
{
    public bool hanging;
   ...
   void Update()
    {
      ...
      if ((Input.GetKeyUp(KeyCode.D) || timer > timeLimit) && hanging)
      {
         hanging = false;
         ...
         {
            player.GetComponent<PlayerBehavior>().RpcDestroyJoint(2);
         }
         else
         {
            player.GetComponent<PlayerBehavior>().CmdDestroyJoint(2);
         }
   ...
   {
      if (this.GetComponent<RightFootBehavior>().enabled == false)
      {
         return;
      }
      ...
      {
         if (Input.GetKey(KeyCode.D) && !hanging && GetComponent<HingeJoint>() == null)
         {
            timer = 0.0f;
            if (player.GetComponent<PlayerBehavior>().isServer)
            {
               player.GetComponent<PlayerBehavior>().RpcCreateJoint(2, transform.position);
            }
            else
            {
               player.GetComponent<PlayerBehavior>().CmdCreateJoint(2, transform.position);
            }
