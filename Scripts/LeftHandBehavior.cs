...
public class LeftHandBehavior : MonoBehaviour
{
   ...
   void Update()
    {
      ...
      if ((Input.GetMouseButtonUp(0) || timer > timeLimit) && hanging)
      {
         hanging = false;
         ...
         {
            player.GetComponent<PlayerBehavior>().RpcDestroyJoint(1);
         }
         else
         {
            player.GetComponent<PlayerBehavior>().CmdDestroyJoint(1);
         }
   ...
   {
      if (this.GetComponent<LeftHandBehavior>().enabled == false)
      {
         return;
      }
      ...
      {
         if (Input.GetMouseButton(0) && !hanging && GetComponent<HingeJoint>() == null)
         {
            timer = 0.0f;
            if (player.GetComponent<PlayerBehavior>().isServer)
            {
               player.GetComponent<PlayerBehavior>().RpcCreateJoint(1, transform.position);
            }
            else
            {
               player.GetComponent<PlayerBehavior>().CmdCreateJoint(1, transform.position);
            }
